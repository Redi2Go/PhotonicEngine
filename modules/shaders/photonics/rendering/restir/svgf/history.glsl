...
SampleHistory sample_history_invalid() {
    ...
    return SampleHistory(vec4(nan), vec4(nan), 1.0f);
}
...
void sample_history_encode(SampleHistory history, out uvec4 value, out float visiblity) {
    ...
    visiblity = history.visibility;
}

void sample_history_decode(out SampleHistory history, uvec4 value, float visibility) {
#if PH_RESTIR_DENOISER_PASSES > 0
...
void sample_history_load(out SampleHistory history, ivec2 texel) {
    sample_history_decode(
            history,
            texelFetch(diffuse_history, texel, 0),
            texelFetch(visibility_history, texel, 0).r
    );
}
