...
#ifndef PH_RESTIR_RESERVOIR_INPUT
#define PH_RESTIR_RESERVOIR_INPUT radiosity_reservoirs
#endif
...
void reservoir_init(inout Reservoir reservoir) {
    int candidate_count = ph_light_count;
    int light_offset = 0;

    #ifdef PH_ENABLE_LIGHT_BINNING
    light_offset = load_light_offset(rt_pos);
    candidate_count = clamp(light_registry_array[light_offset], 0, PH_MAX_SAMPLES);
    #endif

    if (candidate_count == 0) return;

    float light_importance = 1f / float(candidate_count);
    for (int i = 0; i < PH_RESTIR_INITIAL_SAMPLES; i++) {
        if (i >= candidate_count) break;

        #ifdef PH_ENABLE_LIGHT_BINNING
        int rand_index = light_registry_array[light_offset + i + 1];
        #else
...
bool reservoir_reuse(inout Reservoir reservoir, ivec2 uv) {
    ivec2 reservoir_size = textureSize(PH_RESTIR_RESERVOIR_INPUT, 0);
    if (any(lessThan(uv, ivec2(0))) || any(greaterThanEqual(uv, reservoir_size))) return false;
...
bool reservoir_reproject(inout Reservoir reservoir) {
    ...
    ivec2 previous_size = textureSize(prev_radiosity_reservoirs, 0);
    if (any(lessThan(uv, vec2(0f))) || any(greaterThanEqual(uv, vec2(previous_size)))) return false;
...
void sample_history_load(out SampleHistory smple) {
    smple.lighting = texelFetch(radiosity_lighting_raw, tex_coord, 0);
    smple.lighting.a = 1f;
    smple.variance = vec4(0f);
}

SampleHistory sample_history_mix(SampleHistory s1, SampleHistory s2, float a) {
    if (s1 == NULL_HISTORY && s2 == NULL_HISTORY) {
        return NULL_HISTORY;
    } else if (s1 == NULL_HISTORY) {
        a = 1f;
...
SampleHistory sample_history_reproject_single(vec2 uv) {
    ivec2 previous_size = textureSize(prev_radiosity_lighting, 0);
    if (any(lessThan(uv, vec2(0f))) || any(greaterThanEqual(uv, vec2(previous_size)))) return NULL_HISTORY;
