...
#define PH_RESTIR_RESERVOIR_INPUT radiosity_reservoirs_initial
#elif REUSE_ITERATION == 1
#define PH_RESTIR_RESERVOIR_INPUT radiosity_reservoirs_spatial_0
#else
#define PH_RESTIR_RESERVOIR_INPUT radiosity_reservoirs_spatial_1
#endif
...
void main() {
    ...
    if (samples >= min(PH_RESTIR_ACCUMULATION_FRAMES - 1, 4)) {
        reservoir_frag_out = texelFetch(PH_RESTIR_RESERVOIR_INPUT, tex_coord, 0);
    ...
    reservoir_decode(
        reservoir,
        texelFetch(PH_RESTIR_RESERVOIR_INPUT, tex_coord, 0),
        rt_pos,
        false
    );
